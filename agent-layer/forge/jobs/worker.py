...
def run_worker(queue: JobQueue, handler: Handler, *, run_forever: bool = True) -> None:
    ...
    while True:
        ...
        if claimed is not None:
            ...
            logger.info("job %s claimed: %s", job_id, payload.get("instruction", "")[:80])
            try:
...
def main() -> None:
    ...
    redis_client = cast(RedisLike, redis.Redis.from_url(settings.redis_url, decode_responses=True))
